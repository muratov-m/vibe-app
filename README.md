...
- ✅ **Система ролей** - управление ролями пользователей (Admin) и защита API endpoints
...
**Шаг 2:** Примените миграции (роль Admin назначится автоматически):
```bash
cd src/VibeApp.Api
dotnet ef database update
```
...
- `POST /api/auth/assign-role` - Назначить роль (требует роль Admin)
...
### User Profile API
- `POST /api/userprofile/import` - Импорт профиля пользователя из JSON **(требует роль Admin)**
- `GET /api/userprofile` - Получить все профили (публичный)
- `GET /api/userprofile/{id}` - Получить профиль по ID (публичный)
- `PUT /api/userprofile/{id}` - Обновить профиль **(требует роль Admin)**
- `DELETE /api/userprofile/{id}` - Удалить профиль **(требует роль Admin)**

### RAG Search API
- `POST /api/ragsearch/search` - Семантический поиск профилей через RAG (публичный)
- `GET /api/ragsearch/search?q={query}` - Быстрый поиск через GET (публичный)

### Embedding Queue API **(требует роль Admin)**
...
**Authorization API:**
1. Импортируйте коллекцию из `docs/VibeApp-Auth-API.postman_collection.json`
2. Выполните запросы в следующем порядке:
   - **Initialize Admin** - создать первого admin пользователя
   - **Login as Admin** - войти в систему
...
### Система ролей
Приложение использует role-based authorization:
...
- Требуется цифра: Нет
- Требуется строчная буква: Нет
- Требуется заглавная буква: Нет
- Требуется специальный символ: Нет
- Уникальный email: Да

**Примеры валидных паролей:** `123`, `abc`, `admin`, `test`

**Примечание:** В production рекомендуется усилить требования в `Program.cs`.
